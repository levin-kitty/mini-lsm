...
impl LeveledCompactionController {
    ...
    pub fn generate_compaction_task(
        ...
        for i in (0..(self.options.max_levels - 1)).rev() {
            ...
            // why > base_level_size_bytes?? base_level이 아닌데?
        ...
        let mut priorites = Vec::with_capacity(self.options.max_levels);
        for level in 0..self.options.max_levels {
            let prio = real_level_size[level] as f64 / target_level_size[level] as f64;
            if prio > 1.0 {
                priorites.push((prio, level + 1));
            }
        }
        priorites.sort_by(|a, b| a.partial_cmp(b).unwrap().reverse());
        let priority = priorites.first();
        if let Some((_, level)) = priority {
            ...
            println!("compaction triggered by priority: {level} out of {:?}, select {selected_sst} for compaction", priorites);
            return Some(LeveledCompactionTask {
